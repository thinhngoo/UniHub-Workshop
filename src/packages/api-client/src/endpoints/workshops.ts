...
export const workshopsApi = (http: AxiosInstance) => ({
  list: (query: WorkshopListQuery = {}) =>
    http.get<PaginatedResponse<Workshop>>('/workshops', { params: query }).then((r) => r.data),
  ...
  getSummaryJobStatus: (jobId: string) =>
    http
      .get<WorkshopSummaryJobStatusResponse>(`/workshops/summary-jobs/${encodeURIComponent(jobId)}`)
      .then((r) => r.data),
});
